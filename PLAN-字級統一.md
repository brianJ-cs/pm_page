...
**狀態：步驟 1、2、3 都做完了（2026-07-23）。**
步驟 3 收在「整份」分頁：每個版位照真實比例拼成一張，格子裡跑的還是拼板本人
（2cell 的 `ui=stage` 帶 `ro=1`），捲到看得見才掛 iframe、捲走就收（`IntersectionObserver`，
`buildDmView`）。唯讀那條線靠 `d.dm` 跟落版分開走，不回報、不進歷史。
下面的規格和步驟原樣留著 —— 要知道「為什麼這樣做」還是看這一份。
