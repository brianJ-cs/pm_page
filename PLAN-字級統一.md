...
**狀態：步驟 1、2 做完了（2026-07-23）。步驟 3（整份 DM 檢視）做過又整包移掉了。**
步驟 3 曾經收在「整份」分頁（`buildDmView` + `IntersectionObserver` + 2cell 的 `ro=1`
唯讀畫布），2026-07-23 依需求整個拿掉 —— 要重做的話 git 歷史裡還在（commit `c5c533b`）。
