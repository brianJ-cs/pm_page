...
`true`，於是任何存檔動作都會同步上去 —— 包括「壓力測試：塞滿假檔期」那顆按鈕，
它會往正式資料庫塞一筆假檔期。已經發生過一次。
...
`PM_NO_SYNC`）。特別小心「壓力測試」「示範檔期」「清空資料」這種會存檔的按鈕。
...
- 壓力測試的假檔期每一格都塞滿了貨，但 `sheet.confirmed` 全是 false。
  拿 `confirmed` 當「PM 做完了」的條件，畫面就會整片是灰的。
